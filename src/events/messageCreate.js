...
module.exports = async (message) => {
  ...
  const Embed = new EmbedBuilder().setThumbnail(message.guild.iconURL({ dynamic: true })).setColor(message.member.displayHexColor).setFooter({ text: config.activity, iconURL: message.guild.iconURL({ dynamic: true }) }).setTimestamp();
  args = args.splice(1);
